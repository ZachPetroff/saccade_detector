function sac = binsaccT(sacl,sacr);
%--------------------------------------------------------------------
% slightly changed version of original function to account for time overlap
% instead of index overlap
...
for i=1:NL              % loop over left-eye saccades                  
    ...
    l1t = sacl(i,8);    % begin saccade left eye (time)
    l2t = sacl(i,9);    % end saccade left eye (time)

    if NR>0
        R1 = sacr(:,1);    % begin saccade right eye
        R2 = sacr(:,2);    % end saccade right eye

        R1t = sacr(:,8);    % begin saccade right eye (time)
        R2t = sacr(:,9);    % end saccade right eye (time)

        %==================================================
        % testing for temporal overlap with right eye
        %==================================================
        %overlap = find( R2>=l1 & R1<=l2 );    
        overlap = find( R2t>=l1t & R1t<=l2t );    %this is where we're losing the saccades
        if length(overlap)>0
...
%                 phi mean([dxl dxr]) mean([dyl dyr])];
            % changing so the beginning and ends of both right and left eye are included
            s = [min([l1 r1]) max([l2 r2]) ...
                mean([vl vr]) mean([ampl ampr]) ...
                phi mean([dxl dxr]) mean([dyl dyr]) ...
                r1 r2 l1 l2];
